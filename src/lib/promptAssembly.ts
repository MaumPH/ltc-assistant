...
function buildContractInstructions(contract: PromptContract, retrievalMode: RetrievalMode | undefined): string {
  if (contract === 'planner') {
    return [
      ...
      "- Never omit or generalize numeric, deadline, population, or exception qualifiers from evidence. For example, do not rewrite '14일 이내' as '신속히'.",
      '- When a parent instruction and its attached qualifier appear together in evidence, include the qualifier verbatim in the plan or answer contract.',
      '- For broad enumeration queries such as onboarding, offboarding, incidents, or complaints, do not collapse the answer to only one or two indicators when multiple retrieved indicators materially apply.',
  ...
  return [
    ...
    "- Never omit or generalize numeric, deadline, population, or exception qualifiers from evidence. For example, do not rewrite '14일 이내' as '신속히'.",
    '- When a parent instruction and its attached qualifier appear in the same chunk, include the qualifier verbatim in the conclusion, direct evidence, or applicability conditions.',
    '- For broad enumeration queries such as onboarding, offboarding, incidents, or complaints, reflect each distinct retrieved indicator at least once instead of answering from only the highest-scoring one or two indicators.',
