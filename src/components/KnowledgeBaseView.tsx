...
import { allKnowledgeListItems } from '../lib/knowledgeStatic';
...
const CATEGORY_BADGE_STYLES: Record<string, string> = {
  [CATEGORY_ORDER[0]]: 'bg-slate-900 text-white',
  [CATEGORY_ORDER[1]]: 'bg-blue-100 text-blue-700',
  [CATEGORY_ORDER[2]]: 'bg-cyan-100 text-cyan-700',
  [CATEGORY_ORDER[3]]: 'bg-emerald-100 text-emerald-700',
  [CATEGORY_ORDER[4]]: 'bg-amber-100 text-amber-700',
  [CATEGORY_ORDER[5]]: 'bg-violet-100 text-violet-700',
  [CATEGORY_ORDER[6]]: 'bg-slate-100 text-slate-600',
};

const CATEGORY_DOT_STYLES: Record<string, string> = {
  [CATEGORY_ORDER[0]]: 'bg-slate-900',
  [CATEGORY_ORDER[1]]: 'bg-blue-500',
  [CATEGORY_ORDER[2]]: 'bg-cyan-500',
  [CATEGORY_ORDER[3]]: 'bg-emerald-500',
  [CATEGORY_ORDER[4]]: 'bg-amber-500',
  [CATEGORY_ORDER[5]]: 'bg-violet-500',
  [CATEGORY_ORDER[6]]: 'bg-slate-400',
};
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-2 flex flex-wrap items-center gap-1.5 text-[11px] text-slate-500">
              <span className={`rounded-full px-2 py-0.5 font-medium ${CATEGORY_BADGE_STYLES[file.category] ?? 'bg-slate-100 text-slate-600'}`}>
                {file.category}
              </span>
                ...
                <section key={category} className="rounded-[28px] border border-slate-200 bg-white p-3 shadow-sm sm:p-4">
                  ...
                  >
                    <div className="flex items-center gap-2.5">
                      <span className={`h-2.5 w-2.5 rounded-full ${CATEGORY_DOT_STYLES[category] ?? 'bg-slate-400'}`} />
